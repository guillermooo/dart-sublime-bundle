...
def build_comment_data(view, pt):
    ...
    for suffix in suffixes:
        start = all_vars.setdefault("TM_COMMENT_BLOCK_START" + suffix)
        end = all_vars.setdefault("TM_COMMENT_BLOCK_END" + suffix)
        disable_indent = all_vars.setdefault("TM_COMMENT_BLOCK_DISABLE_INDENT" + suffix)

        if start and end:
            block_comments.append((start, end, disable_indent == 'yes'))
            block_comments.append((start.strip(), end.strip(), disable_indent == 'yes'))
        elif start:
            block_comments.append((start, '', disable_indent == 'yes'))
            block_comments.append((start.strip(), '', disable_indent == 'yes'))
...
class DartToggleCommentCommand(sublime_plugin.TextCommand):
    ...
    def remove_block_comment(self, view, edit, comment_data, region):
        ...
        for c in block_comments:
            ...
            if not end:
                comment_data = [[[start, disable_indent]], []]
                if self.remove_line_comment(view, edit, comment_data, region):
                    return True
    ...
    def block_comment_region(self, view, edit, block_comment_data, region):
        ...
        if region.empty():
            if not end:
                self.line_comment_region(view, edit, [start, disable_indent], region)
                return
